...
exports.verifyUser = async (req, res) => {
  try {
    ...
    if (!userOtp) {
      return res.status(400).json({ message: "OTP not found. Please signup again." })
    }
    if (userOtp.otpExpires && userOtp.otpExpires < new Date()) {
    ...
    if (!user) {
      return res.status(400).json({ message: "User not found. Please signup again." })
    }
...
exports.findUser = async (req, res) => {
  try {
    ...
    //checking is otp store in db or not if yes then deleting them and then we will send new otp
...
exports.forgetPassword = async (req, res) => {
  try {
    ...
    const salt = await bcrypt.genSalt(10)
    const existingUserPass = await bcrypt.hash(password, salt);
    existingUser.password = existingUserPass
